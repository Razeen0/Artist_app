...
class User < ApplicationRecord
  ...
  # Auto-provision an artist profile when a new artist user is created,
  # but only when one was not supplied via nested attributes.
  after_create :setup_artist_profile, if: -> { artist? && artist_profile.nil? }

  # Keep the profile name in sync when the user's name changes.
  ...
  def approved_artist?
    artist? && artist_profile&.is_approved
  end
